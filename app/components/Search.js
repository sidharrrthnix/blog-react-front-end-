  ...
  useEffect(() => {
    if (state.requestCount) {
      ...
      async function fetchResult() {
        try {
          const response = await Axios.post("/search", { searchTerm: state.searchTerm }, { token: ourRequest.token })
          setState((draft) => {
      ...
      <div className="search-overlay-bottom">
        <div className="container container--narrow py-3">
          ...
          <div className={"live-search-results" + (state.show === "result" ? "live-search-results--visible" : "")}>
            ...
            {!Boolean(state.results.length) && <p className="alert alert-danger text-center shadow-sm">Sorry, we couldn't find anything</p>}
          </div>
        </div>
      </div>
    </>
